...
def _felzenszwalb8_all_cython(down_cost, right_cost, dright_cost, uright_cost, Py_ssize_t height=285, Py_ssize_t width=381, double scale=1, sigma=0.8,
...
def _felzenszwalb4_all_cython(down_cost, right_cost, Py_ssize_t height=285, Py_ssize_t width=381, double scale=1, sigma=0.8,
                         Py_ssize_t min_size=20):
    """Felzenszwalb's efficient graph based segmentation for
    single or multiple channels.
    ...
    """
    cdef cnp.ndarray[cnp.float_t, ndim=1] costs = np.hstack([
        right_cost.ravel(), down_cost.ravel()]).astype(float)                                                                                                                                                                                                                                                                                                                                              
    ...
    right_edges = np.c_[segments[:, 1:].ravel(), segments[:, :width-1].ravel()]
    cdef cnp.ndarray[cnp.intp_t, ndim=2] edges \
            = np.vstack([right_edges, down_edges])
